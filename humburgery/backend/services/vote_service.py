...
async def change_dish(vote_id: PydanticObjectId, dish_id: PydanticObjectId, session: AsyncClientSession) -> None:
    await vote_repository.change_dish(vote_id, dish_id, session)

    logger.info(f"Vote with id {vote_id} dish was Changed to dish id with id {dish_id}")


async def create_vote(vote: VoteRequest, user_id: PydanticObjectId, session: AsyncClientSession) -> VoteResponse:
    new_vote = await vote_repository.create_vote(vote.dish_id, user_id, session)
